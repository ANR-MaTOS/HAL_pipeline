...
def make_prompt_test(src, template_string, src_lang, tgt_lang, tokenizer=None, no_sys_message = False):
...
def make_prompts_for_inference(src_path, datestamp, src_lang, tgt_lang, max_len, template_string, tokenizer = None, no_sys_message = False):    
    datefile = f"segments_{datestamp}.json"
    with open(Path(src_path) / datefile, "r", encoding="utf-8") as f: 
        data = json.load(f) 
    src_list = []
    abstract_field = f"{src_lang}_abstract_s"
    for entry in data: 
        if "docid" in entry.keys() and "segments" in entry.keys(): 
            src_list.append({"docid":entry["docid"], "src_abstract":entry[abstract_field][0], "segments":entry["segments"]})
    res = []
    for src in src_list:
        docid = src["docid"]
        ...
        for segment in segments:
            prompt_entry = dict()
            rank = segment["rank"]
            src_txt = segment["src_txt"]
            src_tokens = tokenizer.encode(src_txt)
            src_len = len(src_tokens)
            prompt = make_prompt_test(src_txt, template_string, src_lang, tgt_lang, tokenizer = tokenizer, no_sys_message = no_sys_message)
                ...
                for subtask in task.get("subtasks",[]):
                    ...
                    if model.get("chat_template", False):
                        instructions = make_prompts_for_inference(
                            src_path, datestamp, src_lang, tgt_lang, max_len, template_string = model["template"], tokenizer = tokenizer, 
                    ...
                    with open(prompt_filename, "w", encoding="utf-8") as prompt_file:
                        json.dump(instructions, prompt_file, ensure_ascii=False, indent=2)
